...
\begin{itemize}
    ...
    % TODO: check this
    \item \textit{strip-nondeterminism} is a library for stripping non-deterministic information, such as timestamps and file system order, from files. Usage of strip-determinism in the build process can be used to achieve reproducibility. However, it is designed as a quick fix tool, not as permanent solution. In general, it is preferred that software handles reproducibility issues internally, without resorting to third-party tool every time it is built.
